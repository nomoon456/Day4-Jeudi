...
puts "Nombre de journalistes :  #{x}"

n = 0
x = 0

while handle[x]
	results = (handle[x].gsub(/[^0-9]/, '')).to_i
	if results > 0
		n += 1
	end
	x += 1
end
puts "Nombre de Handle contenant un numéro : #{n}"

n = 0
x = 0

while handle[x]
	if handle[x].downcase.include? 'aude'
		n += 1
	end
	x += 1
end
puts "Nombre de Handle contenant \"Aude\" : #{n}"
